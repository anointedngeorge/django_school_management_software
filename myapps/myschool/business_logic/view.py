...
def result_load_student(request, id, query, context):
    try:
        ...
        return TemplateResponse(
            request=request,
            template="admin/templateresponse/admin_add_result.html",
            context=context,
            status=200,
        )
    except Exception as e:
...
def result_edit_student(request, id, query, context):
    try:
        ...
        return TemplateResponse(
            request=request,
            template="admin/templateresponse/admin_edit_result.html",
            context=context,
            status=200,
        )
    except Exception as e:
    ...
    try:
        if request.method == "POST":
            ...
            if foundResults.exists():
                ...
                sections_id = data.get('sections_id')
                ...
                for index, foundResult in enumerate(foundResults):
                    ...
                    for student in foundStudents:
                        if ((STUDENT_CONTEXT.__contains__(student.code)) and (result.get(student.code) != None)):
                            # total = 0
                            student_fullname = f"{student.first_name} {student.middle_name} {student.last_name}"
                            # school information
                            STUDENT_CONTEXT.get(student.code)["school_title"] = "St. Augustin Secondary School"
                            STUDENT_CONTEXT.get(student.code)["school_address"] = "24, ABC street, DEF Road, Enugu State"
                            STUDENT_CONTEXT.get(student.code)["school_phone"] = "9876543210"
                            STUDENT_CONTEXT.get(student.code)["school_email"] = "admin@saphss.com"


                            STUDENT_CONTEXT.get(student.code)["fullname"] = student_fullname
                            STUDENT_CONTEXT.get(student.code)["student_code"] = student.code
                            STUDENT_CONTEXT.get(student.code)["photo"] = student.photo.path
                            STUDENT_CONTEXT.get(student.code)["subject"] = subject
                            STUDENT_CONTEXT.get(student.code)["classes"] = classes
                            STUDENT_CONTEXT.get(student.code)["sections"] = sections
                            STUDENT_CONTEXT.get(student.code)["session"] = session
                            STUDENT_CONTEXT.get(student.code)["term"] = term
                            STUDENT_CONTEXT.get(student.code)["class_no"] = foundStudents.count()
                            total =  result.get(student.code).get('total')
                            STUDENT_CONTEXT.get(student.code)["total"] += int(total)
                            # update STUDENT_CONTEXT_AVERAGE
                            # total no of subjects / by the summation of each subject total = average
                            STUDENT_CONTEXT.get(student.code)["subject_total"] = foundResults.count()
                            STUDENT_CONTEXT_AVERAGE.get(student.code).append(int(total))
                            STUDENT_CONTEXT_TOTAL.get(student.code).append(int(total))
                            STUDENT_CONTEXT.get(student.code)["data"].append(result.get(student.code))
                ...
                for student in foundStudents:
                    ...
                    student_container.append(STUDENT_CONTEXT.get(student.code))
            ...
            context['is_graph_shown'] = True
            context['graph_fields'] = "total"
            context['result_headings'] = "subject,total"
            return TemplateResponse(
                request=request,
                template="admin/templateresponse/termly_results.html",
                context=context,
                status=200,
