...
@register.inclusion_tag('admin_custom/inclusion_pages/edit_result_template.html', 
                        takes_context=True)
def bootstrap_edit_form(context):
    try: 
        return context
    except Exception as e:
        return e
...
@register.inclusion_tag('admin_custom/inclusion_pages/add_result_template.html', 
                        takes_context=True)
def bootstrap_add_form(context):
    try: 
        return context
    except Exception as e:
        return e
...
@register.simple_tag
def get_data_with_index(index, data):
    try: 
        return data[index]
    except Exception as e:
        return e
...
def table_heading(columns='total,'):
    try:
        ...
        table += f"<th>#</th>"
        for x in cols:
            table += f"<th><small>{str(x).upper()}</small></th>"
...
@register.inclusion_tag("admin_custom/results/graph/student_performance.html", takes_context=True)
